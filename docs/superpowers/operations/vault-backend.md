...
| Path | Purpose | Updates |
|---|---|---|
| `data/vault/handbook/{craft,lore}/<slug>.md` | Generated H2 sections — runtime source of truth | Re-run `pnpm migrate-handbook-to-vault` after editing the legacy sources |
| `data/vault/handbook/{spells,monsters,items,rules,classes}/.gitkeep` | Phase 02+ placeholders | Don't add files in Phase 01 — the tool surface doesn't expose them |
| `data/vault/handbook/index.md` | TOC (generated) | Same — regenerated by the migration |
| `data/vault/tools/index.md` + per-tool stubs | Lenient-discovery surface (REQ-012) | Same |
| `data/master_handbook.md` + `data/master_world_lore.md` | LEGACY authoring sources (NOT removed in Phase 01) | Edit these → re-run migration |
