...
- **Tool contract = lazy-loaded** from `/tools/index.md` + `/tools/<name>.md`. Fallback to inline TOOL_CONTRACT_SLIM if compliance gate (G2) fails.
...
| **G2** | Tool discovery compliance ≥90% on at least one M4-realistic model | 002 |
...
| 002 | tool-discovery-compliance | comparison | LLM reads `/tools/<name>.md` before invoking tool — ≥90% across 20 cold turns, multiple models | PENDING | g2, compliance, ollama |
