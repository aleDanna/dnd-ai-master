...
| REQ-045 | Combat encounter events (`combat_start`, `monster_spawn`, `initiative_set`, `turn_advance`, `hp_change`, `monster_hp_change`, `combat_end`) must be applied under SERVER authority and must NOT depend on the local model emitting `apply_event` tool calls mid-conversation. The server OPENS the encounter (deterministic: detect combat intent → select monster stats from the seeded SRD bestiary / CR table → emit `combat_start`+`monster_spawn`+`initiative_set` via `dispatchVaultTool` with `sessionId`), reusing the existing v1/v2 resolvers untouched; the model only narrates. Rule-faithful trigger: initiative is rolled "when violence starts" (master_handbook.md:174, rules.md:155-159, master_handbook_compact.md:58) — the opener fires on detected attack/combat intent, not behind a prior roll. Custom-boss NAMES are preserved; custom STATS derive from SRD/CR (a constrained-JSON LLM opener for bespoke stats is a deferred option-A enhancement on the same hook). | combat resolver redesign, live smoke 2026-05-31 |
| REQ-046 | The combat tracker (turn pointer, PC and monster HP, actor list) must reflect the current encounter state on the vault path within one turn, INCLUDING when the completion SSE event is dropped. The client recovers combat state via session-snapshot refetch (`refetch()`), not only `/messages`+`/character`; server-resolved encounter emits fire a `state` refresh (`dispatchVaultTool` passes `sessionId` → `emitStateRefresh`), and the no-narration combat turn (empty master message) still triggers a tracker refresh. | deferred-items Finding 1, live smoke 2026-05-31 |
| REQ-047 | On a player attack, damage is applied only AFTER the damage roll is submitted and is never inferred from the to-hit roll; narration must not state a damage number before the damage roll. (Already enforced by the v1 two-step resolver — a HIT emits no `monster_hp_change` and returns a `damageRequest`; this REQ locks it as a phase-level invariant the opener work must not bypass.) | live smoke 2026-05-31 |
