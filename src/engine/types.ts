...
export interface Character {
  ...
  /**
   * PHB §6 — in-flight downtime activities the PC has started. Each
   * entry tracks the activity kind (practicing_profession, recuperating,
   * researching, training, crafting), days remaining, and any gp spent.
   * The `crafting` literal is provided for completeness — the actual
   * crafting bookkeeping lives on `craftingProjects` via the Phase 12
   * tools. Empty by default; managed via `start_downtime_activity` /
   * `complete_downtime_activity` mutations. Optional so legacy
   * snapshots without the column still typecheck — the snapshot
   * hydrator defaults it to `[]`.
   */
  downtimeActivities?: DowntimeActivity[];
  /**
   * PHB §6 — currently retained hirelings (skilled or unskilled). Each
   * entry holds the daily wage, count, and total computed cost of the
   * engagement so the master can show the ledger and dismiss them
   * cleanly. Empty by default; managed via `hire` / `dismiss_hireling`
   * mutations.
   */
  hirelings?: Hireling[];
  /**
   * 2024 PHB simplified Bastion — the PC's owned property (basic tier).
   * Holds a name, fortification tier (modest/fortified/castle), an
   * array of rooms, and a defender garrison count. NULL/absent until
...
/**
 * PHB §6 — kinds of downtime activity the engine recognises. The first
 * four are full-fledged activities the master starts/completes via
 * `start_downtime_activity` / `complete_downtime_activity`. The
 * `crafting` literal exists for completeness so the master can speak
 * about a downtime project as crafting; actual crafting bookkeeping
 * lives on `Character.craftingProjects` via the Phase 12 tools.
 */
export type DowntimeActivityKind =
...
/**
 * PHB §6 — an in-flight downtime activity pinned on a Character. Each
 * activity has a stable `id` so the master can address it across turns;
 * the activity is removed from `downtimeActivities` on
 * `complete_downtime_activity`. The master narrates the actual outcome
 * (succeeded recuperation, info gained from research, language
 * learned) — the engine just tracks the days/gp ledger.
 */
export interface DowntimeActivity {
  /**
   * Unique identifier (UUID or slug+timestamp). Generated by the tool
   * layer when emitting `start_downtime_activity`; the applicator just
   * stores it.
   */
  id: string;
  /** Kind of activity. */
  kind: DowntimeActivityKind;
  /** Calendar days still remaining before the activity can be completed. */
  daysRemaining: number;
  /** Cumulative gp already spent on the activity (training pays 1 gp/day). */
  gpSpent: number;
  /**
   * Optional bookkeeping: the in-game timestamp/round the activity was
   * started in. The engine itself does not consume this field — it is
   * preserved so the master can time-stamp downtime cleanly.
   */
  startedAt?: number;
}

/**
 * PHB §6 — a hireling engagement. The master records hirelings via the
 * `hire` tool; the engine computes the gp/sp ledger via
 * `hirelingTotalCost`. Each engagement has a stable `id` so it can be
 * dismissed with `dismiss_hireling`.
 */
export interface Hireling {
  /** Unique identifier (UUID or kind+timestamp). */
  id: string;
  /** PHB §6 wage tier — skilled (2 gp/day) or unskilled (2 sp/day). */
  kind: 'skilled' | 'unskilled';
  /** Number of hirelings in this engagement (>= 1). */
  count: number;
  /** Engagement length in days (>= 1). */
  days: number;
  /** Total computed gp cost across the entire engagement. */
  gpCost: number;
  /** Total computed sp cost across the entire engagement. */
  spCost: number;
  /** Optional: the in-game timestamp/round the engagement started. */
  startedAt?: number;
}

/**
 * 2024 PHB simplified Bastion fortification tier. Drives default room
 * count + defender garrison size via `defaultBastionRooms` /
 * `defaultDefenders` helpers.
 */
export type BastionFortification = 'modest' | 'fortified' | 'castle';

/**
 * 2024 PHB Bastion room kinds. Each kind has a level (1..3) representing
 * basic / improved / master upgrades. The master narrates the actual
 * effects of a given room — the engine only tracks the kind/level so
 * the codex/lookup can render it.
 */
export type BastionRoomKind =
  | 'workshop'
  | 'library'
  | 'armory'
  | 'stable'
...
export interface BastionRoom {
  ...
  /** 1 = basic, 2 = improved, 3 = master. */
  level: 1 | 2 | 3;
}

/**
 * 2024 PHB simplified Bastion record pinned on a Character. A PC owns
 * at most one bastion; `set_bastion` overwrites the entire record while
 * `add_bastion_room` appends to `rooms`.
 */
export interface Bastion {
  /** Display name of the bastion (e.g. "Ravenhollow Manor"). */
  name: string;
  /** Fortification tier — drives default room/defender counts. */
  fortification: BastionFortification;
  /** Rooms currently built (level 1..3 each). */
  rooms: BastionRoom[];
  /** Number of defenders garrisoning the bastion. */
  defenders: number;
}
...
export type Mutation =
  ...
  // Phase 13 — Downtime activities (PHB §6). Append a downtime activity
  // to `characters.downtimeActivities`. The tool layer is responsible
  // for computing daysRemaining up-front via `downtimeRequirements`;
  // the applicator stays permissive (re-applying the same id is a
  // silent no-op so the event log stays replayable).
  | { op: 'start_downtime_activity'; characterId: string; activity: DowntimeActivity }
  // Phase 13 — Remove a downtime activity from
  // `characters.downtimeActivities`. The master narrates the outcome
  // (success, fail, partial) separately. Idempotent on missing id.
  | { op: 'complete_downtime_activity'; characterId: string; activityId: string }
  // Phase 13 — Hirelings (PHB §6). Append a hireling engagement to
  // `characters.hirelings`. Idempotent on duplicate id.
  | { op: 'hire'; characterId: string; hireling: Hireling }
  // Phase 13 — Release a hireling engagement. Idempotent on missing id.
  | { op: 'dismiss_hireling'; characterId: string; hireId: string }
  // Phase 13 — Bastion (2024 PHB simplified). Overwrite the PC's
  // bastion field. Pass `null` to clear it; pass a `Bastion` to
  // establish/replace.
