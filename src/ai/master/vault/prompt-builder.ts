...
function buildRollsBlock(opts: { language?: string; hideDC: boolean }): string[] {
  ...
  lines.push('');
  lines.push('Be PROACTIVE: it is YOUR job to call for the roll — the player will not ask.');
  lines.push('When the player attempts something whose success is uncertain, request the');
  lines.push('roll BEFORE narrating the outcome. Do NOT just narrate what they discover,');
  lines.push('deduce, notice, recall, or persuade — that pre-empts the dice and railroads');
  lines.push('the result. First narrate the PC attempt — the action the player actually');
  lines.push('declared, depicted concretely (what the character is doing/attempting in the');
  lines.push('fiction) — THEN ask for the roll. Do not narrate around the action without');
  lines.push('showing it.');
  lines.push('');
  lines.push('Pick the ability by the player\'s APPROACH, not reflexively. Do NOT default to');
  lines.push('Persuasion for every social or perceptual beat. In particular:');
  lines.push('- Reading a creature\'s intentions, sensing a lie, judging a mood or motive →');
  lines.push('  Insight (WIS). "Understand the goblin\'s intentions" is Insight, NOT Persuasion.');
  lines.push('- Sizing up a threat, spotting a detail, searching a room, noticing an ambush →');
  lines.push('  Perception or Investigation.');
  lines.push('- Recalling lore, identifying a symbol/creature/spell → the relevant');
  lines.push('  knowledge check (History, Arcana, Nature, Religion).');
  lines.push('- Persuasion is for actively swaying an NPC with a sincere appeal; Deception');
  lines.push('  for a lie; Intimidation for a threat — only when the NPC\'s reaction is in');
  lines.push('  doubt, and only if that is what the player actually attempted.');
  lines.push('- Climbing, sneaking, picking a lock, resisting an effect → the matching');
  lines.push('  check or saving throw.');
  lines.push('');
