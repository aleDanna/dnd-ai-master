...
function buildRollsBlock(opts: { language?: string; hideDC: boolean }): string[] {
  ...
  lines.push('the result. Stop your narration at the moment of the attempt and ask for the');
  lines.push('roll instead. Common triggers (not exhaustive):');
  lines.push('- Sizing someone up, reading intentions, spotting a detail, searching a room,');
  lines.push('  noticing an ambush or a lie → Perception or Insight (or Investigation).');
  ...
  lines.push('- Persuading, deceiving, or intimidating an NPC whose reaction is in doubt →');
  lines.push('  Persuasion, Deception, or Intimidation.');
